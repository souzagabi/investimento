...
BEGIN
	...
	set @sql = concat('    SELECT p.idperson INTO IDP FROM tb_persons p INNER JOIN tb_sgcompany sg ON sg.idperson = p.idperson WHERE sg.descricao = "',psgcompany,'"; ');
	set @sql = concat(@sql,'	IF IDP IS NULL THEN ');
	set @sql = concat(@sql,'		INSERT INTO tb_persons (desperson,descpfcnpj) VALUES ("',pdescompany,'","',pdescnpj,'"); ');
	set @sql = concat(@sql,'		SET IDP = LAST_INSERT_ID(); ');
	set @sql = concat(@sql,'        INSERT INTO tb_sgcompany (descricao,idperson) VALUES ("',psgcompany,'",IDP); ');
	set @sql = concat(@sql,'        SET IDC = LAST_INSERT_ID(); ');
	set @sql = concat(@sql,'        INSERT INTO tb_percomp (idperson,idsgcompany) VALUES (IDP,IDC); ');
	set @sql = concat(@sql,'        IF EX = 1 THEN ');
	set @sql = concat(@sql,'			SET MESSAGE = "ERROR: Erro ao gravar registro na tabela Persons"; ');
	set @sql = concat(@sql,'		END IF; ');
	set @sql = concat(@sql,'	ELSE ');
	set @sql = concat(@sql,'		SELECT idsgcompany INTO IDC FROM tb_sgcompany WHERE descricao = psgcompany; ');
	set @sql = concat(@sql,'        IF IDC IS NULL THEN ');
	set @sql = concat(@sql,'			INSERT INTO tb_sgcompany (descricao) VALUES (psgcompany); ');
	set @sql = concat(@sql,'            SET IDC = LAST_INSERT_ID(); ');
	set @sql = concat(@sql,'            INSERT INTO tb_percomp (idperson, idsgcompany) VALUES (IDP, IDC); ');
	set @sql = concat(@sql,'		END IF; ');
	set @sql = concat(@sql,'        IF EX = 1 THEN ');
	set @sql = concat(@sql,'			SET MESSAGE = "ERROR: Erro ao gravar registro na tabela sgcompany"; ');
	set @sql = concat(@sql,'		END IF; ');
	set @sql = concat(@sql,'    END IF; ');
	set @sql = concat(@sql,'	INSERT INTO tb_investiments (iduser, idperson, dtbuy, idsgcompany) VALUES (piduser, IDP, pdtbuy, IDC); ');
	set @sql = concat(@sql,'	SET IDI = LAST_INSERT_ID(); ');
	set @sql = concat(@sql,'	IF EX = 1 THEN ');
	set @sql = concat(@sql,'		SET MESSAGE = "ERROR: Erro ao gravar registro na tabela Investimens"; ');
	set @sql = concat(@sql,'	END IF; ');
	...
	set @sql = concat(@sql,'    IF EX = 1 THEN ');
	set @sql = concat(@sql,'		SET MESSAGE = "ERROR: Erro ao gravar registro na tabela Buys"; ');
	set @sql = concat(@sql,'    END IF;  ');
	set @sql = concat(@sql,'    SELECT idestoque INTO IDE FROM tb_estoques WHERE sgecompany = psgcompany; ');
	set @sql = concat(@sql,'	IF IDE IS NULL THEN ');
	set @sql = concat(@sql,'		INSERT INTO tb_estoques (idperson,idsgcompany,sgecompany,qtdeestoque,prcaverage) VALUES (IDP,IDC,psgcompany, pqtdebuy,pprcaverage); ');
	set @sql = concat(@sql,'		IF EX = 1 THEN ');
	set @sql = concat(@sql,'			SET MESSAGE = "ERROR: Erro ao gravar registro na tabela Estoques"; ');
	set @sql = concat(@sql,'		 END IF; ');
	set @sql = concat(@sql,'	ELSE ');
	set @sql = concat(@sql,'		SELECT qtdeestoque  + pqtdebuy INTO QTDE FROM tb_estoques e WHERE idestoque = IDE; ');
	set @sql = concat(@sql,'		SELECT ((qtdeestoque * prcaverage) + ptlbuy ) / QTDE INTO AVERAGE FROM tb_estoques e WHERE idestoque = IDE; ');
	set @sql = concat(@sql,'		UPDATE tb_estoques ');
	set @sql = concat(@sql,'		SET  ');
	set @sql = concat(@sql,'			qtdeestoque = QTDE, ');
	set @sql = concat(@sql,'			prcaverage = AVERAGE ');
	set @sql = concat(@sql,'		WHERE idestoque = IDE; ');
	set @sql = concat(@sql,'		IF EX = 1 THEN ');
	set @sql = concat(@sql,'			SET MESSAGE = "ERROR: Erro ao atualizar registro na tabela Estoques"; ');
	set @sql = concat(@sql,'		 END IF; ');
	set @sql = concat(@sql,'	END IF; ');

...
BEGIN
	...
	SELECT p.idperson INTO IDP FROM tb_persons p INNER JOIN tb_sgcompany sg ON sg.idperson = p.idperson WHERE p.descpfcnpj = pdescnpj;
	...
	IF IDP IS NULL THEN
		...
		SELECT LAST_INSERT_ID() INTO IDP FROM tb_persons;
        INSERT INTO tb_sgcompany (descricao,idperson) VALUES (psgcompany,IDP);
        SELECT LAST_INSERT_ID() INTO IDC FROM tb_sgcompany;
	...
	INSERT INTO tb_investiments (iduser, idperson,idsgcompany, dtbuy,dtsell) VALUES (piduser,IDP,IDC,pdtbuy,pdtsell);
	...
	INSERT INTO tb_sells (idinvestiment,idperson,idsgcompany,dtsell,qtdesell,prcsell,tlsell,sprcaverage,stptransaction,stipe,tax,lucre)
		VALUES (IDI,IDP,IDC,pdtsell,pqtdesell,pprcsell,ptlsell,psprcaverage,pstptransaction,pstipe,ptax,plucre);
...
BEGIN
    ...
    SET IDI = 0;
    SET IDC = 0;
    SET IDS = 0; SET QTDE = 0;*/
    
	SET @sql = CONCAT('SELECT p.idperson INTO IDP FROM tb_persons p INNER JOIN tb_sgcompany sg ON sg.idperson = p.idperson WHERE p.descpfcnpj = pdescnpj;');
	...
	IF IDP IS NULL THEN
		...
		SET @sql = CONCAT(@sql,'SELECT LAST_INSERT_ID() INTO IDP FROM tb_persons;');
        SET @sql = CONCAT(@sql, 'INSERT INTO tb_sgcompany (descricao,idperson) VALUES (psgcompany,IDP); ');
        SET @sql = CONCAT(@sql,'SELECT LAST_INSERT_ID() INTO IDC FROM tb_sgcompany;');
        SET @sql = CONCAT(@sql,'INSERT INTO tb_percomp (idperson,idsgcompany) VALUES (DP,IDC); ');
        IF EX = 1 THEN
			SET MESSAGE = "ERROR: Erro ao gravar registro na tabela Persons";
		END IF;
	ELSE
	...
	SET @sql = CONCAT(@sql,'SELECT LAST_INSERT_ID() INTO IDI FROM tb_investiments;');
	
	IF EX = 1 THEN
		SET MESSAGE = "ERROR: Erro ao gravar registro na tabela Investimens";
	END IF;
	SET @sql = CONCAT(@sql,'INSERT INTO tb_buys	(idinvestiment, idperson, idsgcompany, dtbuy, qtdebuy, prcbuy, tlbuy, bprcaverage, btptransaction, btipe) ');
...
-- Dump completed on 2021-06-25  6:35:36

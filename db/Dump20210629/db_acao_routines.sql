...
BEGIN
    ...
    SET @sql = CONCAT(@sql,' GROUP BY e.sgecompany ');
...
BEGIN
	...
	SELECT p.idperson INTO IDP FROM tb_persons p WHERE p.descpfcnpj = pdescnpj;
	SELECT sg.idsgcompany INTO IDC FROM tb_sgcompany sg INNER JOIN tb_persons p ON p.idperson = sg.idperson WHERE sg.descricao = psgcompany;
	...
	INSERT INTO tb_investiments (iduser, idperson, dtbuy,idsgcompany) 
    	VALUES (piduser,IDP,pdtbuy,IDC);
	SET IDI = LAST_INSERT_ID();
	
	IF EX = 1 THEN
		SET MESSAGE = "ERROR: Erro ao gravar registro na tabela Investimens";
	END IF;
	INSERT INTO tb_buys	(idinvestiment, idperson, idsgcompany, dtbuy, qtdebuy, prcbuy, tlbuy, bprcaverage, btptransaction, btipe) 
		VALUES (IDI,IDP,IDC,pdtbuy,pqtdebuy,pprcbuy,ptlbuy,pprcaverage,ptptransaction,ptipe);
	...
	IF IDE IS NULL THEN
		INSERT INTO tb_estoques (idperson,idsgcompany,sgecompany,qtdeestoque,prcaverage) 
			VALUES (IDP,IDC,psgcompany, pqtdebuy,pprcaverage);
		IF EX = 1 THEN
...
BEGIN
	...
	/*IF IDC IS NULL THEN
		SET @sql = CONCAT('INSERT INTO tb_sgcompany (descricao,idperson) VALUES ("',psgcompany,,',IDP,');');
		SELECT last_insert_id() INTO IDC FROM tb_sgcompany;
        SET @sql = CONCAT(@sql,'INSERT INTO tb_percomp (idperson, idsgcompany) VALUES (',IDP,IDC,');');
	END IF;*/
	SELECT max(idinvestiment) INTO IDI FROM tb_investiments;
    SET IDI = IDI + 1;
        
	INSERT INTO tb_investiments (idinvestiment,iduser, idperson, dtbuy,dtsell, idsgcompany) 
    	VALUES (IDI,piduser,IDP,pdtbuy,pdtsell,IDC); 
...
-- Dump completed on 2021-06-29  6:05:48
